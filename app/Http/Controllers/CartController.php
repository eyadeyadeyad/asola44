...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        request()->session()->flash('success','تم إضافة المنتج إلى السلة');
        return back();       
    }  
    ...
    public function checkout(Request $request){
        // Check if cart has items (either database or session)
        $hasItems = false;
        if(auth()->check()) {
            $hasItems = Cart::where('user_id', auth()->user()->id)->where('order_id', null)->exists();
        } else {
            $hasItems = !empty(session('cart'));
        }
